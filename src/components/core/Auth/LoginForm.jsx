  ...
  return (
    <form onSubmit={handleSubmit} className='text-white mt-9 flex flex-col gap-5 font-inter'>
      <label className='text-sm'>Email Address <sup className='text-pink-200'>*</sup>
        <input className='block font-medium p-3 w-full rounded-md bg-richblack-700 mt-2' type='email' placeholder='Enter Email Address' name="email" value={formData.email} onChange={changeHandler} />
      </label>
      <label className='text-sm relative'>Password <sup className='text-pink-200'>*</sup>
        <input className='block font-medium p-3 w-full rounded-md bg-richblack-700 mt-2' type={`${showPassword ? 'text' : 'password'}`} placeholder='Enter Password Address' name="password" value={formData.password} onChange={changeHandler} />
